...
from tensorflow.keras.callbacks import EarlyStopping
from tensorflow.keras.initializers import GlorotUniform
import math # Import math for math.isfinite
...
def run_forecast(graph_data):
    ...
    # --- Start of Modified Model Quality Evaluation ---
    calculated_model_quality = 0.0 # Default to 0.0
    try:
        actual_future = target_df.loc[2025:2035]["normalized_value"].tolist()
        predicted_future = [p["normalized_value"] for p in predictions if 2025 <= p["year"] <= 2035]

        if len(actual_future) == len(predicted_future) and len(actual_future) > 0: # Ensure lists are not empty
            raw_r2_score = float(r2_score(actual_future, predicted_future))

            # Define a threshold for "catastrophically bad" R2
            # Any R2 score below this will be set to 0.0
            # Common values are -1.0, -10.0, or -100.0 depending on how "bad" is defined for your domain.
            R2_BAD_THRESHOLD = -100.0

            if math.isfinite(raw_r2_score):
                if raw_r2_score < R2_BAD_THRESHOLD:
                    calculated_model_quality = 0.0
                    print(f"DEBUG: R2 score ({raw_r2_score:.2e}) was below threshold {R2_BAD_THRESHOLD}. Setting quality to 0.0.")
                elif raw_r2_score > 1.0:
                    # Cap R2 at 1.0 if it goes slightly above due to floating point inaccuracies
                    calculated_model_quality = 1.0
                    print(f"DEBUG: R2 score ({raw_r2_score:.4f}) was above 1.0. Setting quality to 1.0.")
                else:
                    # For "normal" R2 scores, round to 4 decimal places
                    calculated_model_quality = round(raw_r2_score, 4)
            else:
                # Handle NaN or Inf if r2_score produces them
                calculated_model_quality = 0.0
                print(f"DEBUG: R2 score was not finite ({raw_r2_score}). Setting quality to 0.0.")
        else:
            # Case where actual_future or predicted_future are empty or unequal lengths
            calculated_model_quality = 0.0
            print("DEBUG: actual_future and/or predicted_future are empty or have unequal lengths. Setting quality to 0.0.")

    except Exception as e:
        print(f"ERROR: R2 evaluation failed unexpectedly: {e}. Setting quality to 0.0.")
        calculated_model_quality = 0.0

    # Assign the handled quality to model_quality
    model_quality = calculated_model_quality
    # --- End of Modified Model Quality Evaluation ---
