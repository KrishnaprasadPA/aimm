...
# Add optional import for meta-learners or DR learners in future extensions
...
def estimate_causal_effects(factors: Dict, links: List[Dict], lag_years=5) -> List[Dict]:
    ...
    for link in links:
        ...
        try:
            ...
            X = panel_df[[f"{c}_conf" for c in confounders]].values if confounders else None
            groups = panel_df['panel_id'].values

            # Future: optionally switch model_y and model_t to DR learners
            model = DynamicDML(
