...
def generate_synthetic_time_series(start_year=1994, end_year=2035, n_factors=3, seed=42):
    ...
    for t in range(n_years):
        for i in range(n_factors):
            ...
            value = 0.7 * base[t, i] + 0.3 * influence + np.random.normal(0, 0.05)
...
if __name__ == "__main__":
    ...
    print("\n=== Ground Truth Weights Matrix ===")
    print(pd.DataFrame(ground_truth, columns=[f"F{j+1}" for j in range(ground_truth.shape[1])], index=[f"F{i+1}" for i in range(ground_truth.shape[0])]))
